...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # MSCA模块特殊处理：参数为(dim, num_heads, sr_ratio)
            # dim应该等于输入通道数
            c1 = ch[f]
            c2 = c1  # MSCA输出通道数等于输入通道数
            args = [c1, *args]  # 将输入通道数作为第一个参数(dim)
        elif m is ECA:
            # C2ECA custom block handling
            c1 = ch[f]       # input channels
            c2 = c1          # output channels (adjust if different)
            args = [c1, *args]
        elif m is CBAM:
            # C2CBAM custom block handling
            c1 = ch[f]       # input channels
            c2 = c1          # output channels (adjust if different)
            args = [c1, *args]
